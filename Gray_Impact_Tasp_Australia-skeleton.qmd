...
of changing HIV treatment coverage, primary prevention coverage, and scale-up of
PrEP.

#### Model equations

To estimate the number of GBMSM living with HIV overtime we estimate the number
of new infections and

#### Input parameters

For each fixed parameter we specified a best estimate and a uniform range of
values. From these ranges we sampled 1000 parameter sets.
...
**A**\$1 million/\$A**10,000**. All analyses and results were generated using R
version XXXX with reproducible code available in an open access repository along
with input parameters and the final results \[ref Github\].
...
Format95 <- function(value, low, up, places = 2) {
  FormatData(value, lower = low, upper = up, rangestr = "95% CI: ", 
...
Beta <- function(incidence, plhiv, propdiag, propart, propvs, effart,
  propprep, effprep){
  return(incidence / (plhiv * 
      (1 - propdiag * propart * propvs * effart) * 
      (1 - propprep * effprep)))
}
...
simulate <- function(numyears, paramsDf) {
  ...
  for (ii in 2:numyears) {
    ...
    infected[ii] <- infected[ii-1]  + 
      newInfects[ii-1] + paramsDf$diagnosed_os[ii]
...
```{r Save paper output}
# Create and save the final plot and tables for the manuscript

# Create a combination plot for manuscript -------------------------------------
...
lprob <- 0.025 # lower and upper probs for quantile function
uprob <- 0.975

# Functions to simply table generation
Format2019 <- function(option, indicator, digits = 0) {
  FormatData(
...
FormatCumulative <- function(option, digits = 0) {
  FormatData(
    (scenarioResultsBest %>% filter(scenario == option))$newInfects %>% sum(), 
    quantile(avertedInfects[[option]], probs = lprob), 
    quantile(avertedInfects[[option]], probs = uprob),
    places = digits)
}
