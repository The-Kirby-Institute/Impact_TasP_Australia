...
calculations was lower but comparable to the number of new notifications
excluding those previously diagnosed overseas over 2009-2019 (598 notifications
...
higher, respectively, than for the baseline scenario. Cumulatively we estimate
there were `r FormatCumulative("base", rangestr = "95% PI: ")` and
...
HIV health care cost for this period would be
`r FormatCosts("base", "hc_base", rangestr="95% PI: ", scale = 6)` million AUD
for the baseline scenario and
`r FormatCosts("scenario", "hc_base", rangestr="95% PI: ", scale = 6)` million
AUD for the counterfactual scenario. This means the increase in ART over
...
`r FormatCosts("base", "lc35", rangestr="95% PI: ", scale = 6)` million AUD for
the baseline scenario to
`r FormatCosts("scenario", "lc35", rangestr="95% PI: ", scale = 6)` million AUD
for the no increase in ART scenario. This is an increase of
`r FormatCostDiffs("lc35", rangestr = "95% PI: ", scale = 6)` million AUD which
more than offsets the increase in HIV healthcare costs giving a total cost
saving of
`r  FormatData((abs(costDifferencesBest$lc35) - abs(costDifferencesBest$hc_base)) / (10^6), quantile((abs(costDifferencesSims$lc35) - abs(costDifferencesSims$hc_base)) / (10^6), probs = lprob), quantile((abs(costDifferencesSims$lc35) - abs(costDifferencesSims$hc_base)) / (10^6), probs = uprob), places = 0, prefix = "$", rangestr = "95%: ")`
...
strategies GBMSM use against HIV infection. However, our model validation showed
the number of people living with HIV overall was accurately reflected by the
calculations suggesting the annual increase in the number of people with HIV was
approximated well despite the simplifying assumptions. We only conducted the
calculations for Australian GBMSM where there is substantial data available
rather than for all populations affected by HIV. Our analysis could easily be
expanded to include other populations if data were available. In all likelihood
including other populations would not change the conclusions of our results and
only change the magnitude of the costs and benefits. The specifications of the
counterfactual scenario where there was no increase in ART proportion since
`r project$projectVars$indexYearART` could also affect the results. Varying the
time the change occurred or incorporating the trends in this proportion prior to
`r project$projectVars$indexYearART` would result in different outcomes for new
infections, healthcare costs and lifetime care and treatment costs. However, the
differences with the baseline for each of these would scale proportionally
meaning there would be little change to primary outcomes of the analyses. The
main limitation is the lack of accurate retrospective cost data for ART and how
these costs changed prior to `r project$projectVars$indexYearART` . We conducted
a scenario analysis to cover the potential effects of changes to treatment costs
for those living with HIV over 2009-2019 and lifetime care and treatment costs
for those newly infected. Even if we take the most expensive treatment cost
scenario and the least expensive lifetime cost scenario the increase in ART
would still be cost saving or at least cost neutral (see Figure). A more
detailed and comprehensive modelling and costing analysis would change the
estimates but is unlikely to change the overall conclusion that the increase in
costs of providing care and treatment is negated by the reduction in lifetime
HIV costs.
