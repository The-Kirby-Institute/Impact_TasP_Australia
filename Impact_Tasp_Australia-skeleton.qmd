...
a difference equation model that incorporates the change in the proportion
...
We also conduced a costing analysis for each scenario from a national health
...
costs. For Australia, one study modelled the potential population level impact
of increased testing and treatment.[@jansson_predicting_2013] Another
longitudinal cohort study using routine health record data extracted from health
services that provide HIV diagnosis and care to gay and bisexual men (GBM) in
the Australian states of New South Wales and Victoria reported substantial
population-level reductions in HIV incidence due to treatment-as-prevention.
[@callander_hiv_2023] However, we are not aware of any retrospective studies
estimating the national population level impact and cost effectiveness of
increasing ART in Australia following changes in guidelines to recommend earlier
initiation of ART.
...
For this analysis we focused on GBM living with HIV in Australia because they
...
A recent health economics study by Lim et al provides estimates for the lifetime cost of
providing HIV care and treatment to people living with HIV from a health
provider perspective in Australia in 2019 Australian dollars
(AUD).[@lim_lifetime_2022] The estimated cost varied from an undiscounted cost
of
...
For the base case we assumed there was no change in costs over 2009-2019 (no
inflation scenario), so the cost of care and treatment for people living with
HIV in 2009 was the same as for 2019. The other scenarios were based on the
geometric mean of the annual CPI inflation rates over 2009-2019 obtained from
the Australian Bureau of Statistics. [@noauthor_consumer_2023] This mean
...
over 2009-2019 increasing annually at $I_R$ (Inflation 1 scenario) and
$2 \times I_R$ (Inflation 2 scenario) and two scenarios were deflationary with
the annual HIV costs decreasing by $I_R$ (Deflation 1 scenario) and
$2 \times I_R$ (Deflation 2 scenario) each year. Using the specified annual
...
multiplying the number on treatment by the annual treatment cost for that year.
Similarly, the lifetime costs for people newly infected with HIV were obtained
by multiplying the number of new infections each year by the associated annual
lifetime cost for that year.
...
# unction to calculate and format the benefits-costs ratio or return on investment. 
calcROI <- function(base, indicator) {
  ...
  return(FormatData(bestROI, lowerROI, upperROI, places = 2, rangestr = ""))
}
...
summaryTable <- tibble(Indicator = character(),
  ...
  bind_rows(tibble(
    ...
    status_quo = calcROI("hc_base", "lc35"),
    counterfactual = NA))
...
costSummaryTable[["No inflation"]] <- c(
  ...
  calcROI("hc_base", "lc35"),
  FormatCostDiffs("lc50", scale = 6),
  calcROI("hc_base", "lc50"))
...
costSummaryTable[["Inflation scenario 1"]] <- c(
  ...
  calcROI("hc_adjusted_i1", "lc0_adjusted_i1"),
  FormatCostDiffs("lc35_adjusted_i1", scale = 6),
  calcROI("hc_adjusted_i1", "lc35_adjusted_i1"),
  FormatCostDiffs("lc50_adjusted_i1", scale = 6),
  calcROI("hc_adjusted_i1", "lc50_adjusted_i1"))
...
costSummaryTable[["Inflation scenario 2"]] <- c(
  ...
  calcROI("hc_adjusted_i2", "lc0_adjusted_i2"),
  FormatCostDiffs("lc35_adjusted_i2", scale = 6),
  calcROI("hc_adjusted_i2", "lc35_adjusted_i2"),
  FormatCostDiffs("lc50_adjusted_i2", scale = 6),
  calcROI("hc_adjusted_i2", "lc50_adjusted_i2"))
...
costSummaryTable[["Deflation scenario 1"]] <- c(
  ...
  calcROI("hc_adjusted_d1", "lc0_adjusted_d1"),
  FormatCostDiffs("lc35_adjusted_d1", scale = 6),
  calcROI("hc_adjusted_d1", "lc35_adjusted_d1"),
  FormatCostDiffs("lc50_adjusted_d1", scale = 6),
  calcROI("hc_adjusted_d1", "lc50_adjusted_d1"))
...
costSummaryTable[["Deflation scenario 2"]] <- c(
  ...
  calcROI("hc_adjusted_d2", "lc0_adjusted_d2"), 
  FormatCostDiffs("lc35_adjusted_d2", scale = 6),
  calcROI("hc_adjusted_d2", "lc35_adjusted_d2"),
  FormatCostDiffs("lc50_adjusted_d2", scale = 6),
  calcROI("hc_adjusted_d2", "lc50_adjusted_d2"))

# Additional calculation for the lowest ROI
calcROI("hc_adjusted_d2", "lc50_adjusted_i2")
...
over 2009-2019. Without the increase in ART the number of people with HIV would
have increased from `r Format2019("base", "infected", rangestr = "95% PI: ")` to
`r Format2019("scenario", "infected", rangestr = "95% PI: ")` by 2019. This
...
million AUD. This cost saving was maintained across various inflationary
costing scenarios.
...
(the counter factual scenario) the model estimated there would have have been
...
HIV health care cost for this period was estimated to be
...
million AUD which more than offsets the increase in HIV healthcare costs over
this period giving a total cost saving of
`r  FormatData((abs(costDifferencesBest$lc35) - abs(costDifferencesBest$hc_base)) / (10^6), quantile((abs(costDifferencesSims$lc35) - abs(costDifferencesSims$hc_base)) / (10^6), probs = lprob), quantile((abs(costDifferencesSims$lc35) - abs(costDifferencesSims$hc_base)) / (10^6), probs = uprob), places = 0, prefix = "$", rangestr = "95%: ")`
million AUD. Such a cost saving was achieved for each discounting scenario (see
@fig-figure1 D) and for each inflationary and deflationary scenario as shown in
tbl-table4. Even when the greatest HIV healthcare cost and the lowest HIV
lifetime cost in tbl-table4 are compared there is still a cost saving.
