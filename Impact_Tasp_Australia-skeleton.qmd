...
2.  This value was calculated using Eq. @eq-incidence, the fixed input
    parameters, the time-varying parameter values for the year 2009 in
    @tbl-table2, and the initial values of people with HIV and the number of
    notifications (excluding those previously diagnosed overseas) reported in
    national surveillance.[@institute_hiv_2022] This value was then constant
    over the calculation period.
3.  The efficacy of ART was based on clinical trial
    results.[@noauthor_initiation_2015; @noauthor_trial_2015;
    @koenig_same-day_2017; @rosen_initiating_2016; @pilcher_effect_2017]
4.  The efficacy of HIV PrEP was based on the results from the PROUD
...
table2Caption <- "Time varying input parameter estimates. The uncertainty 
specifies the range of the multiplicative factor used to generate the uncertainty 
estimates over time. The endnotes provide references and justifications. The 
values for people living with HIV and diagnoses after 2009 are not used in the 
calculations, instead they are used for validation."
...
# ROI Calculations.
calcROI <- function(indicator) {
  
  costsART <- -(cumHEresultsBest %>% filter(scenario == "scenario"))[["hc_base"]] -
      -(cumHEresultsBest %>% filter(scenario == "base"))[["hc_base"]] 
  ...
  lowerROI <- quantile(
    costDifferencesSims[[indicator]] / -costDifferencesSims[["hc_base"]], 
    probs = lprob
  )
  upperROI <- quantile(
    costDifferencesSims[[indicator]] / -costDifferencesSims[["hc_base"]], 
...
brackets is the 95% percentile interval."

# Create a scenario table for the HE results -----------------------------------
costSummaryTable <- tibble(
  "Cost indicator" = character(4),
  "Deflation scenario 2" = character(4),
  "Deflation scenario 1" = character(4),
  "No inflation" = character(4),
  "Inflation scenario 1" = character(4),
  "Inflation scenario 2" = character(4)) 
...
lifetime HIV costs for each discounting and inflation scenario. The range in 
brackets is the 95% percentile interval. "
