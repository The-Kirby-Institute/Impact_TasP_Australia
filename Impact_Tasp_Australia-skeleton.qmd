...
2009-2011 and 2012-2014 periods (see Table S2).
...
| $c^5_{life}$                    | Lifetime cost of providing care and ART post infection in 2019 AUD discounted 3.5%                                                                                                                                                                                                                                                                             | `r FormatData((parametersFixed %>% filter(code_variable == "lifetime_cost_50d"))$estimate, (parametersFixed %>% filter(code_variable == "lifetime_cost_50d"))$lower, (parametersFixed %>% filter(code_variable == "lifetime_cost_50d"))$upper, places = 0, prefix = "$")` | Lifetime cost of providing HIV care and ART post infection in 2019 AUD discounted 5% | 7

1.  Estimates for the number of GBM with HIV over 2009-2019 were obtained using
the HIV cascade methodology used for national surveillance.[@institute_hiv_2022]
The 2009 estimate was as the initial conditions for the model calculations with
the remaining estimates used for validation (see @tbl-table2).

2.  This value was calculated using Eq. @eq-incidence, the fixed input
parameters, the time-varying parameter values for the year 2009 in @tbl-table2,
and the initial values of people with HIV and the number of notifications
(excluding those previously diagnosed overseas) reported in national
surveillance.[@institute_hiv_2022] This value was then constant over the
calculation period.

3.  The efficacy of ART was based on clinical trial results.[@noauthor_initiation_2015; @noauthor_trial_2015;
@koenig_same-day_2017; rosen_initiating_2016; @pilcher_effect_2017]

4.  The efficacy of HIV PrEP was based on the results from the PROUD study.[@mccormack_pre-exposure_2016]
...
Lim et al study[@lim_lifetime_2022] and dividing by 10 to produce a yearly cost. 
...
diagnosed were estimated using the methodology for producing the Australian HIV
cascade for national surveillance.[@institute_hiv_2022]
...
[@noauthor_australian_2020] This is the same data used for viral suppression in
the Australian HIV cascade.

4. The proportion of GBM taking HIV PrEP was assumed to be zero before 2013.
After 2013 we used the proportion eligible for PrEP multiplied by the proportion
eligible taking PrEP as reported in the Annual Trends in Behaviour
report.[@mao_l_annual_2022]

5. The proportion of GBM using condoms was given by the proportion reporting consistent 
condom use in the Annual Trends in Behaviour report and Holt et al.
[@mao_l_annual_2022; @holt_adapting_2016]
...
previously diagnosed overseas and those or people first diagnosed in Australia.
