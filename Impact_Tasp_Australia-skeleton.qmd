...
**Background**: Several clinical trails have established that effective
...
analysed the impact of increased ART on HIV epidemiology and healtcare costs in
Australia.
...
reviewed previous estimates for HIV care and treatment costs in
Australia.[@schneider_cost-effectiveness_2014, @health_pharmaceutical_2017] One
study estimated costs in 2013 AUD and this was updated to 2015 costs for a
Pharmaceuticals Benefits Scheme application. The estimated costs for each line
of ART compared to the Lim et al analysis are shown in Table S1 in the
...
2012) of individual drugs. The change in reporting from packs to scripts makes
it difficult to determine any trend in price over the 2009-2015 period. Looking
at key drugs that formed part of ART regimens showed varying trends with some
remaining at a similar price while others increased or decreased over the
2009-2011 and 2012-2015 periods (see Table S2).
...
and in our counter-factual scenario we assumed this percentage was fixed from
2011 to 2019.
...
parameters and the final results.[@gray_kirby_nodate].
...
| $c^0_{life}$                    | Annual cost of providing care and ART post infection in 2019 AUD                   | `r FormatData((parametersFixed %>% filter(code_variable == "lifetime_cost_0d"))$estimate, (parametersFixed %>% filter(code_variable == "lifetime_cost_0d"))$lower, (parametersFixed %>% filter(code_variable == "lifetime_cost_0d"))$upper, places = 0, prefix = "$")`    | 7                                                                                    |
| $c^{3.5}_{life}$                | Undiscounted lifetime cost of providing care and ART post infection in 2019 AUD    | `r FormatData((parametersFixed %>% filter(code_variable == "lifetime_cost_35d"))$estimate, (parametersFixed %>% filter(code_variable == "lifetime_cost_35d"))$lower, (parametersFixed %>% filter(code_variable == "lifetime_cost_35d"))$upper, places = 0, prefix = "$")` | 7                                                                                    |
| $c^5_{life}$                    | Lifetime cost of providing care and ART post infection in 2019 AUD discounted 3.5% | `r FormatData((parametersFixed %>% filter(code_variable == "lifetime_cost_50d"))$estimate, (parametersFixed %>% filter(code_variable == "lifetime_cost_50d"))$lower, (parametersFixed %>% filter(code_variable == "lifetime_cost_50d"))$upper, places = 0, prefix = "$")` | Lifetime cost of providing HIV care and ART post infection in 2019 AUD discounted 5% |
...
6.  The estimated annual cost of providing care and treatment to people with HIV
    ...
    cost.
7.  The lifetime HIV costs were each discounting rate were obtained from Lim et
...
table2Caption <- "Time varying input parameter estimates. The assumed
certainty range specifies the multiplicative factor used to generate uncertainty 
estimates. Endnotes provide references and justifications. The values for PLHIV 
and diagnoses after 2009 are not used in the calculations, rather they are used 
for validation."
...
combinedPlotCaption <- "Summary of results from the baseline and no ART increase 
...
to reported notifications of HIV in MSM (excluding those previoulsy diagnosed 
...
costTableCaption <- "Comparison of additional treatment costs and reduction in lifetime
HIV costs for each discounting and inflation scenario."
...
affect the results. Varying the time the change occurred or incorporating the
...
retrospective cost data for ART and how these costs changed since `r
project$projectVars$startYear`. We conducted a scenario analysis to cover the
...
unlikely to change the overall conclusion that the increase in costs of
...
1. The 2013 cost estimates came from Schnieder et
...
2013.[@noauthor_pharmaceutical_2020] The estimate for third and higher lines of
ART is the average of the costs for third and fourth line ART.
...
reported costs were doubles because this study used a six month timestep. First
line cost is equal to the average of the three regimens considered.

\newpage

**Table S2** - Example Cost data for selected ART drugs used over 2009-2011 and
2012-2014 from the Australian Statistics on Medicines. The costs were obtained 
by dividing the total cost by the number of packs (for 2009-2011) or scripts 
(2012-2015) reported.

<!--# Table added manually to the rendered Word document -->
